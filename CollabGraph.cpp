...
vector<Artist> CollabGraph::get_vertex_neighbors(const Artist &artist) const
{
    ...
    Vertex *node = graph.at(artist.get_name());
    int nSize = node->neighbors.size();
    if (!node->neighbors.empty()) {
        for (int i = 0 ; i < nSize; i++) {
            result.push_back(node->neighbors[i].neighbor);
        }
    }
                                            ...
                                            const Artist &dest) const
{
    ...
    stack<Artist> path;
    Vertex *aVert = graph.at(dest.get_name());
    if (aVert->predecessor) {
        path.push(aVert->artist);
        Vertex *curr = aVert;
        while (curr->predecessor) {
            path.push(curr->predecessor->artist);
